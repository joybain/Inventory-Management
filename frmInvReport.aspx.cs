...
public partial class frmInvReport : System.Web.UI.Page
{
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        ...
        else if (RadioButtonList1.SelectedValue == "CPI")
        {
            if (ddlReportType.SelectedValue == "DR")
            {
                ...
                if (rbCurDatePayment.Checked)
                {
                    ...
                    DataTable dt = _aSalesManager.GetCutomerPaymet(DateTime.Now.ToString("dd/MM/yyyy"), DateTime.Now.ToString("dd/MM/yyyy"), hfCustomerIDPayment.Value);
                    if (dt.Rows.Count > 0)
                    {
                ...
                else if (rbByDatePayment.Checked)
                {
                    ...
                    DataTable dt = _aSalesManager.GetCutomerPaymet(txtStartDatePayment.Text, txtEndDatePayment.Text, hfCustomerIDPayment.Value);
                    if (dt.Rows.Count > 0)
                    {
                        CustomerPaymentReport(DDT, dt);
                    }
        ...
        else if (RadioButtonList1.SelectedValue == "CD")
        {
            if (ddlReportType.SelectedValue == "DR")
            {
                ...
                {
                    DDT = "  On " + DateTime.Now.ToString("dd/MM/yyyy") + "";
                    DataTable dt1 = _aSalesManager.GetSaleInfo(DateTime.Now.ToString("dd/MM/yyyy"), DateTime.Now.ToString("dd/MM/yyyy"), hfCustomerIDPayment.Value);
                    DataTable dt = _aSalesManager.GetCutomerPaymet(DateTime.Now.ToString("dd/MM/yyyy"), DateTime.Now.ToString("dd/MM/yyyy"), hfCustomerIDPayment.Value);
                    if (dt1.Rows.Count > 0)
                    {
                ...
                {
                    DDT = "From " + txtStartDate.Text + " to " + txtEndDate.Text + "";
                    DataTable dt1 = _aSalesManager.GetSaleInfo(txtStartDatePayment.Text, txtEndDatePayment.Text, hfCustomerIDPayment.Value);
                    DataTable dt = _aSalesManager.GetCutomerPaymet(txtStartDatePayment.Text, txtEndDatePayment.Text, hfCustomerIDPayment.Value);
                    if (dt1.Rows.Count > 0)
                    {
                        CustomerPaymentANdDueReport(DDT, dt, dt1);
    ...
    private void PurchaseDetailsCurrentDate(string p, string CurrentDate, string ReportType)
    {
        ...
        {
            dt = PurchaseVoucherManager.GetShowPurchaeReport(CurrentDate, txtStartDate.Text, txtEndDate.Text,
               ddlSupplier.SelectedValue, rbReportType.SelectedValue);
        }
    ...
    private void SalesDetailsCurrentDate(string Heading, string Date, string Type)
    {
        ...
        if (Type.Equals("C"))
        {
            ...
            {
                dt = PurchaseVoucherManager.GetShowSalesReportReport("3",
                    DateTime.Now.ToString("dd/MMM/yyyy"), DateTime.Now.ToString("dd/MMM/yyyy"),
                    hfCustomerID.Value, Session["BranchId"].ToString());
        ...
        else
        {
            ...
            if (rbItemWise.SelectedValue == "Item")
            {
                dt = PurchaseVoucherManager.GetShowSalesReportReport("3", txtStartDate.Text,
            ...
            {
                dt = PurchaseVoucherManager.GetShowSalesReportReport(rbReportType.SelectedValue, txtStartDate.Text,
                    txtEndDate.Text, hfCustomerID.Value, Session["BranchId"].ToString());
                ReturnAmount = PurchaseVoucherManager.GetBranchShowreturnAmount(txtStartDate.Text,
                    txtEndDate.Text, hfCustomerID.Value,Session["BranchId"].ToString());
            }
    ...
    //            PayAmount = _aSalesManager.GetSalesPaymentDetails(DateTime.Now.ToString("dd/MM/yyyy"), DateTime.Now.ToString("dd/MM/yyyy"), CustomerID);
    //        }
    //        else
    //            {
    //                PayAmount = _aSalesManager.GetSalesPaymentDetails(txtStartDate.Text, txtEndDate.Text, CustomerID);
    //            }
    //        cell = new PdfPCell(FormatHeaderPhrase("Total"));
    //        cell.HorizontalAlignment = 2;
    //        cell.VerticalAlignment = 1;
    ...
    private void SalesReport(string Heading, DataTable dt, decimal ReturnAmount)
    {
        ...
        foreach (DataRow dr in dt.Rows)
        {
            ...
            taxamount += (Convert.ToDouble(dr["TotalPrice"]) / 100) * Convert.ToDouble(dr["TaxAmount"]); 
        ...
        if (rbItemWise.SelectedValue != "Item")
        {
            ...
            pdtdtl.AddCell(cell);
           // var taxamount = Convert.ToDecimal(PayAmount.Rows[0]["TaxAmount"].ToString());
            var disamount = Convert.ToDecimal(PayAmount.Rows[0]["DiscountAmount"].ToString());
            var csRecved = Convert.ToDecimal(PayAmount.Rows[0]["CashReceived"].ToString());
            var PayAmo = Convert.ToDecimal(PayAmount.Rows[0]["PayAmt"].ToString());
            //var de = (Convert.ToDecimal(tot) + taxamount) - Convert.ToDecimal(ReturnAmount)-  (disamount+csRecved+PayAmo);

            Double Due = (tot + Convert.ToDouble(taxamount)) - Convert.ToDouble(ReturnAmount) - (Convert.ToDouble(PayAmount.Rows[0]["DiscountAmount"]) + Convert.ToDouble(PayAmount.Rows[0]["CashReceived"]) + Convert.ToDouble(PayAmount.Rows[0]["PayAmt"]));
            cell = new PdfPCell(FormatHeaderPhrase(Convert.ToDouble(PayAmount.Rows[0]["TaxAmount"]).ToString("N2")));
            cell.HorizontalAlignment = 2;
            cell.VerticalAlignment = 1;
            cell.BorderColor = BaseColor.LIGHT_GRAY;
    ...
    private void CustomerPaymentReport(string Heading, DataTable dt)
    {
        ...
        dtempty.AddCell(cell);
        document.Add(dtempty);
        float[] dtlw = new float[3] { 20, 70, 15 };
