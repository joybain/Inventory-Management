...
namespace Delve
{
    public class GlCoaManagerRP
    {
        ...
        public static decimal getCoaBalanceFromExistingDataTable(DataTable dt, string book, string glcoacode,
            string sepTyp, string status, string fromdt, string todt, string StartDt1, int? ReportType)
        {
            ...
            DataTable dtNewTable;
                ...
                foreach (DataRow dr in dtAlready.Rows)
                {
                    ...
                    else if (dtFixCode.Rows[0]["ClosingStockInRDALipa_Tow"].ToString().Trim().Equals(dr["gl_coa_code"].ToString().Trim()))
                    {
            ...
            if (dtAlready.Columns.Count > 0)
            {
                foreach (DataRow drdtA in dtAlready.Rows)
                {
                    string expr = "gl_coa_code='" + drdtA["gl_coa_code"].ToString().Trim() + "' and Flag='" + ReportType +
                                  "' ";
                    DataRow[] drtmp = null;
                    try
                    {
                        drtmp = dt.Select(expr);
                        dtdtl = new DataTable();
                        dtdtl = dt.Clone();
                        foreach (DataRow dr in drtmp)
                        {
                        ...
                        if (dtdtl.Rows.Count > 0)
                        {
                            foreach (DataRow drdtl in dtdtl.Rows)
                            {
                                ...
                                bal += decimal.Parse(drdtl["amount_dr"].ToString());
                            }
                        }
                        if (dtdtl != null)
                            dtdtl.Dispose();
        ...
        public static DataTable getCoaBalanceFromExistingDataTable_WithSingleLedger(DataTable dt, string book, string glcoacode,
          string sepTyp, string status, string fromdt, string todt, string StartDt1, int? ReportType)
        {
            string criteria = getCoaCriteria(glcoacode, sepTyp, book);
            DataTable dtAlready = GlCoaManager.GetGlCoaCodes(criteria);
            ...
            for (int ij = 1; ij < 3; ij++)
            {
                ...
                if (dtAlready.Columns.Count > 0)
                {
                    foreach (DataRow drdtA in dtAlready.Rows)
                    {
                        string expr = "gl_coa_code='" + drdtA["gl_coa_code"].ToString().Trim() + "' and Flag='" +
                                      ij +
                                      "' ";
                        DataRow[] drtmp = null;
                        try
                        {
                            ...
                            foreach (DataRow dr in drtmp)
                            {
                                ...
                                {
                                    totAmount += Convert.ToDouble(dr["amount_dr"].ToString());
                                }
                                dtdtl.ImportRow(dr);
                            }
                        }
                ...
                if (ij.Equals(1))
                {
                    ...
                    //drRp["gl_coa_code"] = "";
                    //drRp["COA_NATURAL_CODE"] = "";
                    drRp["SEG_COA_DESC"] = "Total Receipts : ";
                    drRp["amount_dr"] = totAmount.ToString("N3");
                    drRp["Flag"] = "11";
                    dtdtl.Rows.Add(drRp);
                }
                if (ij.Equals(2))
                {
                    ...
                    //drRp["gl_coa_code"] = "";
                    //drRp["COA_NATURAL_CODE"] = "";
                    drRp["SEG_COA_DESC"] = "Total Payments : ";
                    drRp["amount_dr"] = totAmount.ToString("N3");
                    drRp["Flag"] = "11";
                    dtdtl.Rows.Add(drRp);
                }
        ...
        public static string getCoaCriteria(string glCode,string separ,string book)
        {
            ...
            for (int i = 0; i < segcode.Length; i++)
            {
                ...
                else if (x > 0 && segcode[i].ToString() != "")
                {
                    ...
                    " union all select tt" + x.ToString() + ".seg_coa_code,tt" + x.ToString() + ".seg_coa_desc,tt" + x.ToString() + ".parent_code,tt" + x.ToString() + ".ROOTLEAF from GL_SEG_COA as tt" + x.ToString() + " ,t" + x.ToString() + " " +
                    " where t" + x.ToString() + ".seg_coa_code=tt" + x.ToString() + ".PARENT_CODE and tt" + x.ToString() + ".LVL_CODE='" + lvlcode[x] + "') ";
                    criteria += " and substring(gl_coa_code,charindex('" + sep + "',gl_coa_code," + y + ")+1," + lvlSize[x].ToString() + ") in (SELECT SEG_COA_CODE  FROM t" + x.ToString() + " where rootleaf='L' ) ";
                }
                y = y + lvlSize[x];
                x = x + 1;
            }
